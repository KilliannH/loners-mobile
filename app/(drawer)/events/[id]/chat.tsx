...
export default function ChatRoomScreen() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get<{ messages: ChatMessage[] }>(`/chat/${eventId}`);
  ...
  const markAsRead = useCallback(async () => {
    ...
    markAsReadStore(eventId);
  }, [eventId, markAsReadStore]);
      ...
      return (
        ...
        >
          ...
          {isMe && showAvatar && (
            ...
            >
              {user?.avatarUrl ? (
                <Image source={{ uri: user.avatarUrl }} style={{ width: 28, height: 28 }} />
              ) : (
                <Text
                  style={{
                    fontSize: 10,
