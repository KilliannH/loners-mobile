...
export default function EditEventScreen() {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        const tType = (availableTypes.includes(ev.type as any) ? ev.type : "autre") as
          (typeof availableTypes)[number];
        setType(tType);

        setInitialName(ev.name || "");
        setInitialDescription(ev.description || "");
        setInitialType(tType);
        ...
        Alert.alert(t("eventDetails.errorTitle"), t("editEvent.loadError"));
        router.back();
      } finally {
        setLoading(false);
      }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Retour aux détails
      router.push({ pathname: "/event_details/[id]", params: { id: String(eventId) } });
    } catch (e: any) {
      console.log(e?.response?.data || e?.message);
      Alert.alert(t("eventDetails.errorTitle"), t("editEvent.updateError"));
    } finally {
      setSaving(false);
    }
  }, [canSubmit, id, name, description, type, combined, router, t, eventId]);
  ...
  const Header = (
    <View style={{ padding: 16 }}>
      ...
      >
        ...
        <Text style={{ marginLeft: 6, color: "#4b5563" }}>{t("common.back")}</Text>
      </TouchableOpacity>
      ...
      {!isFuture && (
        <Text style={{ color: "#ef4444", marginTop: 8 }}>{t("editEvent.futureError")}</Text>
      )}
    </View>
  );
