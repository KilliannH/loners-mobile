...
export default function Login() {
  ...
  return (
    <View style={{ flex:1, backgroundColor:"#f3f4f6", padding:20, justifyContent:"center" }}>
      <Text style={{ fontSize:28, fontWeight:"bold", textAlign:"center", marginBottom:20 }}>Welcome back 👋</Text>
      ...
      <TouchableOpacity onPress={submit} disabled={loading} style={{ borderRadius:8, overflow:"hidden" }}>
        <LinearGradient colors={["#2563eb","#4f46e5"]} start={{x:0,y:0}} end={{x:1,y:0}}
          ...
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={{ color:"#fff", fontWeight:"bold", fontSize:16 }}>Login</Text>}
        </LinearGradient>
      </TouchableOpacity>
    </View>
  );
